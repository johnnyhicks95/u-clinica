...
class DoctoresM extends ConexionBD{
	...
	static public function CrearDoctorM($tablaBD, $datosC){
		...
		$pdo = ConexionBD::cBD()->prepare("INSERT INTO $tablaBD(apellido, nombre, sexo, id_consultorio, 
				usuario, clave, rol) VALUES(:apellido, :nombre, :sexo, :id_consultorio, :usuario, :clave, :rol)");
		...
		if($pdo -> execute()){
			return true;  // retorna true para la condicion del controlador
		}
	...
	static public function ActualizarDoctorM($tablaBD, $datosC){
		...
		$pdo = ConexionBD::cBD()->prepare("UPDATE $tablaBD SET apellido = :apellido, nombre = :nombre, 
									sexo = :sexo, usuario = :usuario, clave = :clave WHERE id = :id");
